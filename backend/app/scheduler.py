...
from app.services.renewal_service import renew_due_subscriptions
...
async def _renewal_job() -> None:
    try:
        async with async_session_maker() as session:
            n = await renew_due_subscriptions(session)
            if n:
                logger.info("[scheduler] recorded %s renewal payments", n)
    except Exception as exc:
        logger.warning("[scheduler] renewal failed: %s", exc)
...
def start_scheduler() -> None:
    ...
    # 구독 자동 갱신 + 결제 내역 기록: 매일 00:10 UTC (≈ 09:10 KST)
    _scheduler.add_job(_renewal_job, "cron", hour=0, minute=10, id="renewal")
    _scheduler.add_job(_renewal_job, "date", id="renewal_initial")  # 시작 직후 1회
